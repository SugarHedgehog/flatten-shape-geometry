...
export default class Trapezoid extends Quadrilateral {
    constructor({ lengths = {}, height = 0, angles = {}, supplementary = {} }) {
        ...
        switch (true) {
            case Object.keys(lengths).length === 4:
                ...
                break;
            case Object.keys(lengths).length >= 2 && Object.keys(angles).includes('angle') && height != undefined:
                this._isAngleInDegree = angles.angleInDegree || false;
                const angleKey = Object.keys(angles.angle || angles)[0];
                const angleValue = angles.angle ? angles.angle[angleKey] : angles[angleKey];
                const angleInRadians = this._isAngleInDegree ? degreesToRadians(angleValue) : angleValue;
    ...
    _trapezoidByLengths(lengths) {
        Object.keys(lengths).forEach((key) => {
            ...
            if (!Number.isFinite(Number(lengths[key]))|| lengths[key]<=0)
    ...
    _trapezoidByTwoBasesHeightAngle(lengths, height, angleKey, angleInRadians) {
        ...
        switch (angleKey) {
            ...
            case 'angleB':
                ...
                xOffset = lengthAB - (height / Math.tan(angleInRadians)) - lengthCD;
                break;
            case 'angleC':
                // If angle C is known, calculate offset from the right side
                xOffset = lengthAB - (height / Math.tan(Math.PI - angleInRadians)) - lengthCD;
                break;
            case 'angleD':
